...
function getBreadcrumb(pathname) {
  ...
  const map = {
    ...
    '/forensic/requests': 'Seize Requests',
...
export default function ForensicLayout() {
  ...
  return (
    <div className="app-wrapper">
      ...
      <aside className={`sidebar forensic-sidebar${collapsed ? ' collapsed' : ''}${mobileOpen ? ' mobile-open' : ''}`} id="forensicSidebar" role="navigation" aria-label="Forensic Navigation">
        <div className="sidebar-logo-area" style={{ background: 'linear-gradient(135deg,#00bcd4,#015C94)' }}>
          <div className="sidebar-logo-circle" style={{
            width: 44, height: 44, borderRadius: '50%', background: 'rgba(255,255,255,0.16)',
        ...
        <div className="sidebar-brand-text">
          <div className="brand-name"><b>FORENSIC</b></div>
          <div className="brand-sub"><b>NCCIA Digital Forensic Lab</b></div>
        </div>
        <nav className="sidebar-nav" aria-label="Forensic Sidebar Navigation">
          <div className="nav-section-label">Forensic</div>
          <div className="nav-item">
            <NavLink to="/forensic" end className="nav-link" data-page="forensic-dashboard">
      ...
      <div className={`main-content${collapsed ? ' collapsed' : ''}`}>
        <header className="header" style={{ background: 'linear-gradient(90deg,#0097a7,#015C94)' }}>
          <div className="header-left">
            <button className="sidebar-toggle" id="forensicSidebarToggle" title="Toggle Sidebar" aria-label="Toggle Sidebar" onClick={toggleSidebar}>
            ...
            <nav className="breadcrumb" aria-label="breadcrumb">
              ...
              <span style={{color:'#fff'}}>Forensic Lab</span>
              <span style={{color:'#fff'}} className="separator">›</span>
          ...
          <div className="header-right">
            ...
            <div ref={userMenuRef} style={{position:'relative'}}>
              <div className={`header-user${userMenuOpen ? ' open' : ''}`} onClick={() => setUserMenuOpen(!userMenuOpen)} style={{cursor:'pointer'}}>
                <div className="header-avatar" style={{ background: 'rgba(255,255,255,0.22)' }}>{initials}</div>
                ...
                <div className="user-dropdown-body">
                  ...
                  {isAdmin && (
                    <a href="#users" className="user-dropdown-item" role="menuitem" onClick={(e) => { e.preventDefault(); go('/forensic/users'); }}>
                      <div className="user-dropdown-item-icon" style={{background:'#0097a7',color:'#fff'}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></div>
