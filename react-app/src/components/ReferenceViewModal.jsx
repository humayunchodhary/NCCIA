export default function ReferenceViewModal({ open, title, item, fields, onClose }) {
  ...
  return (
    ...
    >
      <div
        style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 640, maxHeight: '85vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 14, marginBottom: 16 }}>
            {fields.filter(f => f.key !== 'description').map(f => (
              <div key={f.key}>
                ...
                <div style={{ marginTop: 4, fontSize: 13, fontWeight: 600 }}>{item[f.key] || '—'}</div>
              </div>
            ))}
          </div>
          {(item.description || item.content) && (
            <div style={{ padding: 14, background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 13, lineHeight: 1.55, whiteSpace: 'pre-wrap' }}>
              {item.description || item.content}
            </div>
          )}
