...
export default function ForensicLogin() {
  const { forensicLogin, error, remaining, retryAfter, clearError, user } = useAuth();
      ...
      <div className="card" style={{position:'relative',zIndex:10,display:'flex',width:'min(900px, 95vw)',minHeight:'560px',borderRadius:'24px',overflow:'hidden',boxShadow:'0 20px 40px rgba(0,0,0,0.2), 0 0 0 1px #2B2B2B',animation:'cardRise .7s cubic-bezier(.22,1,.36,1) both'}}>
        ...
        <div className="panel-left" style={{flex:1,background:'#2B2B2B',padding:'48px 36px',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden',color:'#fff'}}>
          ...
          <ul className="info-list" style={{listStyle:'none',width:'100%',display:'flex',flexDirection:'column',gap:'14px'}}>
            ...
            <li style={{display:'flex',alignItems:'flex-start',gap:'12px',fontSize:'.85rem',lineHeight:1.5,color:'rgba(255,255,255,0.95)'}}>
              ...
              <span>Restricted portal — access limited to authorized forensic personnel only</span>
            </li>
            <li style={{display:'flex',alignItems:'flex-start',gap:'12px',fontSize:'.85rem',lineHeight:1.5,color:'rgba(255,255,255,0.95)'}}>
              <span className="icon-dot" style={{width:'28px',height:'28px',flexShrink:0,background:'rgba(0,0,0,0.25)',borderRadius:'8px',display:'flex',alignItems:'center',justifyContent:'center'}}><svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" width="16" height="16"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0110 0v4"/><circle cx="12" cy="16" r="1"/></svg></span>
              <span>Unauthorized access is strictly prohibited and will be prosecuted</span>
            </li>
            <li style={{display:'flex',alignItems:'flex-start',gap:'12px',fontSize:'.85rem',lineHeight:1.5,color:'rgba(255,255,255,0.95)'}}>
              ...
              <span>Confidentiality of access credentials is solely the responsibility of the officer.</span>
            </li>
          </ul>
        ...
        <div className="panel-right" style={{flex:1,background:'#fff',padding:'48px 42px',display:'flex',flexDirection:'column',justifyContent:'center',position:'relative',overflow:'hidden'}}>
          ...
          <div className="form-header" style={{marginBottom:'32px'}}>
            <h2 style={{fontFamily:"'Playfair Display', serif",fontSize:'1.65rem',color:'#2B2B2B',fontWeight:700}}>Forensic Portal</h2>
            <p style={{marginTop:'6px',fontSize:'.83rem',color:'#264078'}}>Sign in with your NCCIA Forensic Officer credentials</p>
          </div>
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group" style={{marginBottom:'20px'}}>
              <label htmlFor="forensic-email" style={{display:'block',fontSize:'.75rem',fontWeight:600,letterSpacing:'.06em',textTransform:'uppercase',color:'#2B2B2B',marginBottom:'7px'}}>Email Address</label>
            ...
            <div className="form-group" style={{marginBottom:'20px'}}>
              <label htmlFor="forensic-password" style={{display:'block',fontSize:'.75rem',fontWeight:600,letterSpacing:'.06em',textTransform:'uppercase',color:'#2B2B2B',marginBottom:'7px'}}>Password</label>
              <div className="input-wrap" style={{position:'relative'}}>
                <span className="icon" style={{position:'absolute',left:'14px',top:'50%',transform:'translateY(-50%)',color:'#264078',pointerEvents:'none',display:'flex'}}>
                ...
                <input type={showPwd ? 'text' : 'password'} id="forensic-password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••••••" autoComplete="current-password" required style={{width:'100%',padding:'12px 14px 12px 42px',border:'1.5px solid #264078',borderRadius:'11px',fontSize:'.88rem',fontFamily:"'DM Sans', sans-serif",color:'#2B2B2B',background:'#fff',outline:'none'}} />
            ...
            <button type="submit" className="btn-login" style={{width:'100%',padding:'13px',background:'#015C94',border:'none',borderRadius:'12px',color:'#fff',fontFamily:"'DM Sans', sans-serif",fontSize:'.92rem',fontWeight:600,letterSpacing:'.04em',cursor:'pointer',boxShadow:'0 4px 12px rgba(0,0,0,0.3)'}}>Sign In to Forensic Portal</button>
          </form>
          <p className="help-text" style={{marginTop:'20px',textAlign:'center',fontSize:'.78rem',color:'#264078'}}>Need access? <a href="#" style={{color:'#2B2B2B',textDecoration:'none',fontWeight:500}}>Contact NCCIA Forensic Admin</a></p>
